...
# Anchors measured on Celeb-DF v2:
#   Real videos avg: 0.54 → maps to 0 after normalisation
#   Fake videos avg: 0.93 → maps to 1 after normalisation
# YouTube deepfakes score lower (0.60-0.75) due to heavy re-encoding.
# Thresholds are set conservatively to catch compressed internet fakes.
REAL_ANCHOR      = 0.54   # real video anchor (Celeb-DF v2 measured)
FAKE_ANCHOR      = 0.93   # fake video anchor (Celeb-DF v2 measured)
NORM_THRESHOLD   = 0.25   # lowered from 0.50 — catches compressed internet fakes
FRAME_THRESHOLD  = 0.58   # lowered from 0.73 — individual frame guard
FAKE_FRAME_RATIO = 0.30   # lowered from 0.60 — 30% of frames must confirm
...
def smart_verdict(scores):
    ...
    # Primary condition: normalised score above threshold
    # Secondary condition: enough frames confirm (catches compressed videos too)
    # OR: if norm score is very high (>0.65), trust it even with fewer frames
    primary   = norm_avg > NORM_THRESHOLD
    secondary = fake_ratio > FAKE_FRAME_RATIO
    strong    = norm_avg > 0.65   # very high score — confident fake regardless of frame ratio

    is_fake = primary and (secondary or strong)
    verdict = "FAKE" if is_fake else "REAL"

    # Confidence: how far from threshold in confirmed direction
            ...
            if not frames:
                ...
                if 0.20 < norm_score < 0.45:
                    ...
                    <div style="background:rgba(245,166,35,0.08);border:1px solid rgba(245,166,35,0.3);
                                ...
                                color:#f5a623;letter-spacing:1.5px;line-height:1.8;">
                        &#9888;&nbsp;BORDERLINE RESULT — Model confidence is low for this video.
                        Score is near the decision threshold. This may indicate subtle manipulation,
                        heavy compression, or a video type outside the training distribution.
                    </div>""", unsafe_allow_html=True)
                ...
                <div style="background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.05);
                            ...
                            color:#333;letter-spacing:1px;line-height:1.8;">
                    &#9432;&nbsp;DISCLAIMER — NoCap is trained on DFDC and Celeb-DF v2 datasets.
                    Performance may vary on videos with heavy compression, unusual lighting,
                    multiple faces, very short duration, or deepfakes generated by methods
                    not in the training data (e.g. diffusion models, 2023+). This tool is
                    indicative only and should not be used as sole evidence in any legal,
                    editorial, or official decision-making process.
                </div>""", unsafe_allow_html=True)
